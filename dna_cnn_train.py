...
class SmallDNACNN(nn.Module):
    ...
    def __init__(self, in_ch: int, n_classes: int, p_drop: float=0.3):
        super().__init__()
...
def train_eval(args):
    ...
    # Model
    model = SmallDNACNN(in_ch, n_classes, p_drop=0.3).to(device)
...
if __name__ == "__main__":
    # Your earlier interface kept; defaults adjusted to be *less aggressive* and small-data friendly
    ...
    p.add_argument("--mutate-rate", type=float, default=0.005)        # gentler
    p.add_argument("--rc-prob", type=float, default=0.5)
    p.add_argument("--pairing-prob", type=float, default=0.0)         # MixUp OFF by default
